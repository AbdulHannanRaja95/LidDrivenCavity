...
while(((er >= 1e-5) && (t <= 10000)))%error condition is always true....time condition gets violated which results in stopage of code 
    
    %u1,v1 and p1  are computed at different time from actual u,v and p. u1,v1 and p1 are actually storage variable.they store the data 
    %computed at each and every iteration
    u1 = u;
    v1 = v;
    p1 = p; 
    % pressure calculation from continuity eqaution with artificial compressibility factor incorporated
    for i=2:nx-1%to incorporate the center difference scheme for fist order derivative of u in x direction
        for j=2:ny-1%to incorporate the center difference scheme for fist order derivative of u in y direction
            p(i,j)=p1(i,j) - dt*b*(0.5/dx*(u(i+1,j)-u(i-1,j))+0.5/dy*(v(i,j+1)-v(i,j-1))); %FT for prssure term, CS for convective terms
    ...
    p(:,1) = p(:,2);
    p(:,ny) = p(:,ny-1);
    % This part solves x-momentum equation in non-dimensional form..CS scheme is used for Second order derivative terms and also for the
    % first order term.
    for i=2:nx-1
         for j=2:ny-1
            ...
            u(i,j) = u1(i,j) + dt*RHSu - dt*LHSu; %FT for time derivative term, u is calculated at u(n+1) while u1 is calculated at u1(n)
         end
    end
    
    % This part solves y-momentum equation in non-dimensional form..CS scheme is used for Second order derivative terms and also for the
    % first order term.
    for i=2:nx-1
        for j=2:ny-1
            ...
            v(i,j) = v1(i,j) + dt*RHSv - dt*LHSv;%FT for time derivative term, again v(n+1) and v1(n)
        end
    end
